      ...
      if (c == 'p')
        {
          ...
          PARSER_READ_NUM (num, c);
          num_clauses = num;
          clausegroups = (ClauseGroupID*)malloc(num_clauses * sizeof(ClauseGroupID));
...
clock_t t2;
  ...
  */
  ...
  ClauseGroupID* unsat_core = qdpll_get_relevant_clause_groups (depqbf);
  ...
  int i;
  /*
  printf("UNSAT-Core hat größe %ld\n",sizeof(unsat_core));
  for(i=0;i<=sizeof(unsat_core);i++){
    printf("%d\n",unsat_core[i]);
  }
  */
  
  int new_size=0;
  while (unsat_core[new_size] != 0) {
        //printf("%u\n", *ptr);
        new_size++;
  }
